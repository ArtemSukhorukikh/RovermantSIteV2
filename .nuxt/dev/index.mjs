...
import { PrismaClient } from 'file://G:/ProgramsSiteV2/node_modules/@prisma/client/default.js';
...
const _lazy_2oTTEJ = () => Promise.resolve().then(function () { return delete_post$1; });
const _lazy_ALn4zC = () => Promise.resolve().then(function () { return update_post$1; });
...
const _lazy_h9u5w0 = () => Promise.resolve().then(function () { return addCondition_post$1; });
...
const handlers = [
  ...
  { route: '/api/stateProgram/addCondition', handler: _lazy_h9u5w0, lazy: true, middleware: false, method: "post" },
...
const _id__get$a = defineEventHandler(async (event) => {
  ...
  const company = prisma$e.company.findFirst({ where: { id: Number(id) } });
  if (company === null) {
    throw createError({
      status: 404,
...
const all_get$2 = defineEventHandler(async (event) => {
  const companies = prisma$d.company.findMany();
  return { companies };
});
...
const create_post$2 = defineEventHandler(async (event) => {
  ...
  const user = await prisma$c.user.findFirst({ where: { id: body.userId } });
  if (user === null) {
    throw createError({
      status: 404,
  ...
  const company = await prisma$c.company.create({
    data: {
      name: body.name,
...
const delete_post = defineEventHandler(async (event) => {
  ...
  const { count } = await prisma$b.company.deleteMany({
    where: {
      id: {
        in: body.ids
      }
...
const update_post = defineEventHandler(async (event) => {
  const body = await readBody(event);
  const user = await prisma$a.user.findFirst({ where: { id: body.userId } });
  if (user === null) {
    throw createError({
      status: 404,
      message: "\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D"
  ...
  let company = await prisma$a.company.findFirst({ where: { id: body.id } });
  if (company === null) {
    throw createError({
      status: 404,
      message: "\u041F\u0440\u0435\u0434\u043F\u0440\u0438\u044F\u0442\u0438\u0435 \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D\u043E"
  ...
  await prisma$a.companyCondition.deleteMany({
    where: { companyId: body.id }
  });
  company = await prisma$a.company.update({
    where: {
      id: body.id
    },
    data: {
...
const _id__get$8 = defineEventHandler(async (event) => {
  ...
  const user = await prisma$9.user.findFirst({ where: { id: Number(id) } });
  if (!user) {
    throw createError({
      status: 400,
      message: "\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D"
  ...
  const companies = await prisma$9.company.findMany({
    include: {
      user: true
    },
...
const _id__get$6 = defineEventHandler(async (event) => {
  ...
  const user = await prisma$8.user.findFirst({ where: { id: Number(id) } });
  if (!user) {
    throw createError({
      status: 400,
      message: "\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D"
  ...
  const userCompanies = await prisma$8.company.findMany({
    where: { userId: Number(id) },
    include: {
      conditions: {
        include: {
  ...
  const programs = await prisma$8.stateProgram.findMany({
    include: {
      conditions: true
    }
  });
...
const register_post = defineEventHandler(async (event) => {
  ...
  let user = await prisma$6.user.findFirst({
    where: {
      email: body.email
    }
  });
...
const _id__get$4 = defineEventHandler(async (event) => {
  ...
  const statePrograms = prisma$5.stateProgram.findFirst({
    where: { id: Number(id) }
  });
...
const addCondition_post = defineEventHandler(async (event) => {
  ...
  const stateProg = await prisma$4.stateProgram.findFirst({
    where: {
      id: body.programId
    }
  });
  ...
  const condition = prisma$4.condition.create({ data: {
...
const all_get = defineEventHandler(async (event) => {
  const statePrograms = prisma$3.stateProgram.findMany({
    include: {
      conditions: true
    }
  });
...
const create_post = defineEventHandler(async (event) => {
  const body = await readBody(event);
  const state = await prisma$2.stateProgram.create({
    data: {
      name: body.name,
  ...
  if (body.conditions) {
    for (const condition of body.conditions) {
      await prisma$2.condition.create({
        data: {
...
const _id__get$2 = defineEventHandler(async (event) => {
  ...
  const programs = await prisma$1.stateProgram.findMany({
    include: {
      conditions: true
    }
  });
  const companies = await prisma$1.company.findMany({
    where: { userId: Number(id) },
    include: {
      conditions: {
        include: {
