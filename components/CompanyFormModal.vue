...
async function onSubmit(event) {
    ...
    if (id.value !== "") {
        const { error } = await $fetch("/api/company/update", {
            method: "POST",
            body: {
                id: id.value,
...
<template>
    <UModal v-model="modalOpen">
        <UCard :ui="{
            ...
            <template #header>
                <span v-if="!company.name" class="font-medium text-3xl">Добавление нового предприятия</span>
                <span v-else class="font-medium text-3xl">Редактировать педприятие</span>
            </template>
            ...
            <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
                ...
                <UButton class="mr-12" color="indigo" variant="outline" type="submit">
                    Сохранить
                </UButton>

                <UButton color="indigo" variant="outline" type="button" @click="close()">
                    Закрыть
                </UButton>
            </UForm>
        </UCard>
